...
new_preamble = """
  ...
  # PLAY NIM SECTION
    Nim is a two-player game. Here are the complete rules:

  SETUP:
  The game starts with 6 stones (barrels).
  Players alternate turns.
  On each turn, a player must remove either 1 or 2 stones from the pile per turn.
  The player who is forced to take the last stone has lost the match.

  HOW YOU PLAY:
  To play optimally, always try to leave your opponent with a number of stones that is a multiple of 3 plus 1.
  For example if there is 4 stones left, take 1 stone (leaving 3 for opponent, but if they take 2 you take 1, and they get stuck with 1 counting as a loss to them).
  Specifically count remaining stones.
  Stones can be considered N
  If (N-1) mod 3 == 2, take 2.
  If (N-1) mod 3 == 1, take 1.
  Otherwise take 1.

  GAME PROMPTING:
  When asked to play, say "Sure, lets play Nim" and output "#NimStart" without the quotes
  When asked who goes first, check the camera. 
  If the closest barrel is blue, say "I'll go first." If the barrel is orange , say "You go first."
  Always keep track of how many stones are left in the game.
  When it is your turn, announce how many you are taking, then physically pick up and move 1 or 2 barrels. 
  To physically pick up and move 1 barrel, output "#NimPickup1" without quotes
  To pyhsically pick up and move 2 barrels, output "#NimPickup2" without quotes
  After your turn, say how many stones remain.
  When a player takes the last stone, that player LOSES.
  React appropriately when the game ends using #act.
...
class CelesteL5(StateMachineProgram):
    ...
    class CmdDrop(StateNode):
      ...
      def setup(self):
          ...
          # Code generated by genfsm on Wed May  6 13:15:16 2026:
    ...
    class MoveToBarrel(StateNode):
        def start(self , event=None):
            ...
            if lowestDistance == 0:
                ...
                self.post_data(lowestDistance + 30)
    ...
    def setup(self):
        ...
        #         dispatch =D(re.compile('#NimStart'))=> StartNim
        #         dispatch =D(re.compile('#NimPickup1'))=> FindBarrel1
        #         dispatch =D(re.compile('#NimPickup2'))=> FindBarrel2
        #         dispatch =C=> loop
        # 
        ...
        # 
        # 
        #         StartNim: self.ArucoName()
        #         StartNim =D=> TurnToward() =C=> self.ArucoDistance() =D=> Forward() =CNext=> dispatch
        #         StartNim =F=> Print("Can't Find Marker") =C=> StartNim
        # 
        #         FindBarrel2: self.LocateBarrel()
        #         FindBarrel2 =D=> TurnToward() =C=> GrabBarrel2
        #         FindBarrel2 =F=> Print("No Barrel Found") =T(5)=> FindBarrel2
        # 
        #         GrabBarrel2: self.MoveToBarrel()
        #         GrabBarrel2 =D=> Forward() =C=> Turn(180) =C=> Forward(300) =C=> Kick() =C=> Turn(180) =C=> self.ArucoName() =D=> TurnToward() =C=> self.ArucoDistance() =D=> Forward() =C=> self.ResetHeading() =D=> Turn() =C=> FindBarrel1 
        #         GrabBarrel2 =F=> GrabBarrel2
        # 
        #         FindBarrel1: self.LocateBarrel()
        #         FindBarrel1 =D=> TurnToward() =C=> GrabBarrel1
        #         FindBarrel1 =F=> Print("No Barrel Found") =T(5)=> FindBarrel1
        # 
        #         GrabBarrel1: self.MoveToBarrel()
        #         GrabBarrel1 =D=> Forward() =C=> Turn(180) =C=> Forward(300) =C=> Kick() =C=> Turn(180) =C=> self.ArucoName() =D=> TurnToward() =C=> self.ArucoDistance() =D=> Forward() =C=> self.ResetHeading() =D=> Turn() =CNext=> dispatch 
        #         GrabBarrel1 =F=> GrabBarrel1
        # 
        ...
        StartNim = self.ArucoName() .set_name("StartNim") .set_parent(self)
        turntoward1 = TurnToward() .set_name("turntoward1") .set_parent(self)
        arucodistance1 = self.ArucoDistance() .set_name("arucodistance1") .set_parent(self)
        forward1 = Forward() .set_name("forward1") .set_parent(self)
        print3 = Print("Can't Find Marker") .set_name("print3") .set_parent(self)
        FindBarrel2 = self.LocateBarrel() .set_name("FindBarrel2") .set_parent(self)
        turntoward2 = TurnToward() .set_name("turntoward2") .set_parent(self)
        print4 = Print("No Barrel Found") .set_name("print4") .set_parent(self)
        GrabBarrel2 = self.MoveToBarrel() .set_name("GrabBarrel2") .set_parent(self)
        ...
        aruconame1 = self.ArucoName() .set_name("aruconame1") .set_parent(self)
        turntoward3 = TurnToward() .set_name("turntoward3") .set_parent(self)
        arucodistance2 = self.ArucoDistance() .set_name("arucodistance2") .set_parent(self)
        forward4 = Forward() .set_name("forward4") .set_parent(self)
        resetheading1 = self.ResetHeading() .set_name("resetheading1") .set_parent(self)
        turn3 = Turn() .set_name("turn3") .set_parent(self)
        FindBarrel1 = self.LocateBarrel() .set_name("FindBarrel1") .set_parent(self)
        turntoward4 = TurnToward() .set_name("turntoward4") .set_parent(self)
        print5 = Print("No Barrel Found") .set_name("print5") .set_parent(self)
        GrabBarrel1 = self.MoveToBarrel() .set_name("GrabBarrel1") .set_parent(self)
        forward5 = Forward() .set_name("forward5") .set_parent(self)
        turn4 = Turn(180) .set_name("turn4") .set_parent(self)
        forward6 = Forward(300) .set_name("forward6") .set_parent(self)
        kick2 = Kick() .set_name("kick2") .set_parent(self)
        turn5 = Turn(180) .set_name("turn5") .set_parent(self)
        aruconame2 = self.ArucoName() .set_name("aruconame2") .set_parent(self)
        turntoward5 = TurnToward() .set_name("turntoward5") .set_parent(self)
        arucodistance3 = self.ArucoDistance() .set_name("arucodistance3") .set_parent(self)
        forward7 = Forward() .set_name("forward7") .set_parent(self)
        resetheading2 = self.ResetHeading() .set_name("resetheading2") .set_parent(self)
        turn6 = Turn() .set_name("turn6") .set_parent(self)
        ...
        datatrans25 = DataTrans(re.compile('#NimStart')) .set_name("datatrans25")
        datatrans25 .add_sources(dispatch) .add_destinations(StartNim)
        
        datatrans26 = DataTrans(re.compile('#NimPickup1')) .set_name("datatrans26")
        datatrans26 .add_sources(dispatch) .add_destinations(FindBarrel1)
        
        datatrans27 = DataTrans(re.compile('#NimPickup2')) .set_name("datatrans27")
        datatrans27 .add_sources(dispatch) .add_destinations(FindBarrel2)
        ...
        datatrans28 .add_sources(StartNim) .add_destinations(turntoward1)
        
        completiontrans12 = CompletionTrans() .set_name("completiontrans12")
        completiontrans12 .add_sources(turntoward1) .add_destinations(arucodistance1)
        
        datatrans29 = DataTrans() .set_name("datatrans29")
        datatrans29 .add_sources(arucodistance1) .add_destinations(forward1)
        
        cnexttrans16 = CNextTrans() .set_name("cnexttrans16")
        cnexttrans16 .add_sources(forward1) .add_destinations(dispatch)
        
        failuretrans6 = FailureTrans() .set_name("failuretrans6")
        failuretrans6 .add_sources(StartNim) .add_destinations(print3)
        
        completiontrans13 = CompletionTrans() .set_name("completiontrans13")
        completiontrans13 .add_sources(print3) .add_destinations(StartNim)
        
        datatrans30 = DataTrans() .set_name("datatrans30")
        datatrans30 .add_sources(FindBarrel2) .add_destinations(turntoward2)
        
        completiontrans14 = CompletionTrans() .set_name("completiontrans14")
        completiontrans14 .add_sources(turntoward2) .add_destinations(GrabBarrel2)
        
        failuretrans7 = FailureTrans() .set_name("failuretrans7")
        failuretrans7 .add_sources(FindBarrel2) .add_destinations(print4)
        
        timertrans1 = TimerTrans(5) .set_name("timertrans1")
        timertrans1 .add_sources(print4) .add_destinations(FindBarrel2)
        
        datatrans31 = DataTrans() .set_name("datatrans31")
        datatrans31 .add_sources(GrabBarrel2) .add_destinations(forward2)
        
        completiontrans15 = CompletionTrans() .set_name("completiontrans15")
        completiontrans15 .add_sources(forward2) .add_destinations(turn1)
        
        completiontrans16 = CompletionTrans() .set_name("completiontrans16")
        completiontrans16 .add_sources(turn1) .add_destinations(forward3)
        
        completiontrans17 = CompletionTrans() .set_name("completiontrans17")
        completiontrans17 .add_sources(forward3) .add_destinations(kick1)
        
        completiontrans18 = CompletionTrans() .set_name("completiontrans18")
        completiontrans18 .add_sources(kick1) .add_destinations(turn2)
        
        completiontrans19 = CompletionTrans() .set_name("completiontrans19")
        completiontrans19 .add_sources(turn2) .add_destinations(aruconame1)
        
        datatrans32 = DataTrans() .set_name("datatrans32")
        datatrans32 .add_sources(aruconame1) .add_destinations(turntoward3)
        
        completiontrans20 = CompletionTrans() .set_name("completiontrans20")
        completiontrans20 .add_sources(turntoward3) .add_destinations(arucodistance2)
        
        datatrans33 = DataTrans() .set_name("datatrans33")
        datatrans33 .add_sources(arucodistance2) .add_destinations(forward4)
        
        completiontrans21 = CompletionTrans() .set_name("completiontrans21")
        completiontrans21 .add_sources(forward4) .add_destinations(resetheading1)
        
        datatrans34 = DataTrans() .set_name("datatrans34")
        datatrans34 .add_sources(resetheading1) .add_destinations(turn3)
        
        completiontrans22 = CompletionTrans() .set_name("completiontrans22")
        completiontrans22 .add_sources(turn3) .add_destinations(FindBarrel1)
        
        failuretrans8 = FailureTrans() .set_name("failuretrans8")
        failuretrans8 .add_sources(GrabBarrel2) .add_destinations(GrabBarrel2)
        
        datatrans35 = DataTrans() .set_name("datatrans35")
        datatrans35 .add_sources(FindBarrel1) .add_destinations(turntoward4)
        
        completiontrans23 = CompletionTrans() .set_name("completiontrans23")
        completiontrans23 .add_sources(turntoward4) .add_destinations(GrabBarrel1)
        
        failuretrans9 = FailureTrans() .set_name("failuretrans9")
        failuretrans9 .add_sources(FindBarrel1) .add_destinations(print5)
        
        timertrans2 = TimerTrans(5) .set_name("timertrans2")
        timertrans2 .add_sources(print5) .add_destinations(FindBarrel1)
        
        datatrans36 = DataTrans() .set_name("datatrans36")
        datatrans36 .add_sources(GrabBarrel1) .add_destinations(forward5)
        
        completiontrans24 = CompletionTrans() .set_name("completiontrans24")
        completiontrans24 .add_sources(forward5) .add_destinations(turn4)
        
        completiontrans25 = CompletionTrans() .set_name("completiontrans25")
        completiontrans25 .add_sources(turn4) .add_destinations(forward6)
        
        completiontrans26 = CompletionTrans() .set_name("completiontrans26")
        completiontrans26 .add_sources(forward6) .add_destinations(kick2)
        
        completiontrans27 = CompletionTrans() .set_name("completiontrans27")
        completiontrans27 .add_sources(kick2) .add_destinations(turn5)
        
        completiontrans28 = CompletionTrans() .set_name("completiontrans28")
        completiontrans28 .add_sources(turn5) .add_destinations(aruconame2)
        
        datatrans37 = DataTrans() .set_name("datatrans37")
        datatrans37 .add_sources(aruconame2) .add_destinations(turntoward5)
        
        completiontrans29 = CompletionTrans() .set_name("completiontrans29")
        completiontrans29 .add_sources(turntoward5) .add_destinations(arucodistance3)
        
        datatrans38 = DataTrans() .set_name("datatrans38")
        datatrans38 .add_sources(arucodistance3) .add_destinations(forward7)
        
        completiontrans30 = CompletionTrans() .set_name("completiontrans30")
        completiontrans30 .add_sources(forward7) .add_destinations(resetheading2)
        
        datatrans39 = DataTrans() .set_name("datatrans39")
        datatrans39 .add_sources(resetheading2) .add_destinations(turn6)
        
        cnexttrans17 = CNextTrans() .set_name("cnexttrans17")
        cnexttrans17 .add_sources(turn6) .add_destinations(dispatch)
        
        failuretrans10 = FailureTrans() .set_name("failuretrans10")
        failuretrans10 .add_sources(GrabBarrel1) .add_destinations(GrabBarrel1)
