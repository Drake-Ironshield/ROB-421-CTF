...
class barrelGamePosition(StateNode):
    ...
    def start(self, event=None):
        ...
        for obj in self.tracker.blue_registry.values():
            if obj is not None:
                if obj.pose.x < blue_win_thresh and obj.pose.y < blue_win_thresh:
                    print("Adding - " , obj.pose.x , " " , obj.pose.y)
                    self.tracker.blue_cap.append(obj)
                else:
                    print("Uncap Adding - " , obj.pose.x , " " , obj.pose.y)
                    self.tracker.uncap.append(obj)
        ...
        #print(self.tracker.orange_registry)
...
### main state machine program
class BarrelTrackerTest(StateMachineProgram):
    def __init__(self):
        # tracking lists
        self.orange_cap = []
        self.blue_cap = []
    ...
    def setup(self):
        #         check: barrelGamePosition(self)
        #         print_node: PrintResultsNode()
        #         
        #         check =D=> print_node
        #         print_node =T(1)=> check
        #         check =F=> StateNode() =T(1)=> check
        
        # Code generated by genfsm on Mon Jun  8 16:47:17 2026:
        
        check = barrelGamePosition(self) .set_name("check") .set_parent(self)
        print_node = PrintResultsNode() .set_name("print_node") .set_parent(self)
        statenode1 = StateNode() .set_name("statenode1") .set_parent(self)
        
        datatrans1 = DataTrans() .set_name("datatrans1")
        datatrans1 .add_sources(check) .add_destinations(print_node)
        
        timertrans1 = TimerTrans(1) .set_name("timertrans1")
        timertrans1 .add_sources(print_node) .add_destinations(check)
        
        failuretrans1 = FailureTrans() .set_name("failuretrans1")
        failuretrans1 .add_sources(check) .add_destinations(statenode1)
        
        timertrans2 = TimerTrans(1) .set_name("timertrans2")
        timertrans2 .add_sources(statenode1) .add_destinations(check)
